...
export default function App() {
  ...
  const [artistSubmissionStatus, setArtistSubmissionStatus] = useState<"idle" | "uploading" | "sending" | "success" | "error">("idle");
  const [artistSubmissionMessage, setArtistSubmissionMessage] = useState("");
  ...
  useEffect(() => {
    ...
    async function fetchJson(endpoint: string, cache: RequestCache) {
      try {
    ...
    async function loadCultureStories(showLoading = false) {
      ...
      const base = import.meta.env.BASE_URL || "./";
      const staticFeedUrl = `${base.replace(/\/$/, "")}/editorial-feed.json`;
      const apiFeedUrl = "/api/editorial-feed?limit=12";

      // Two sources are enough: the deployed static feed and the connected API.
      // The previous implementation tried three equivalent static URLs plus the
      // API and forced every request to bypass the browser cache. That created
      // unnecessary network work on every visit and refresh.
      const [staticData, apiData] = await Promise.all([
        fetchJson(staticFeedUrl, "default"),
        fetchJson(apiFeedUrl, "no-store"),
      ]);
      ...
      const validEmptyFeedSeen = staticData !== null && staticData.stories.length === 0;
  ...
  const storiesBy = (term: RegExp) => editorialStories.filter((story: any) => term.test(`${story.category || ""} ${story.headline || ""} ${story.title || ""} ${story.dek || ""} ${story.source_title || ""} ${story.source_excerpt || ""}`));
  ...
  const allMusicStories = storiesBy(/^(MUSIC)$/i).concat(storiesBy(/music|artist|album|single|afrobeats|hip-hop|ep|song/i)).filter((story: any, index: number, list: any[]) => list.findIndex((item) => storyKey(item) === storyKey(story)) === index);
  const nonRepeatedMusicStories = allMusicStories.filter((story: any) => !heroAndLatestKeys.has(storyKey(story)));
  // Keep the New Music desk useful even when the newest music story is also
  // one of the three latest stories. Prefer a distinct story, but fall back
  // to the strongest music stories rather than showing an empty panel.
  const musicStories = nonRepeatedMusicStories.length ? nonRepeatedMusicStories : allMusicStories.filter((story: any) => storyKey(story) !== heroKey);
  ...
  const uploadArtistFile = async (file: File, artistName: string, kind: "audio" | "artwork") => {
    const maxBytes = kind === "audio" ? 4 * 1024 * 1024 : 2 * 1024 * 1024;
    if (file.size > maxBytes) {
      throw new Error(`${kind === "audio" ? "Audio" : "Artwork"} file is too large. Maximum size is ${kind === "audio" ? "4 MB" : "2 MB"}. You can use the streaming/download link for larger songs.`);
    }
    const allowed = kind === "audio"
      ? /^(audio\/(mpeg|mp3|wav|x-wav|mp4|x-m4a|aac|ogg|webm))$/i
      : /^image\/(jpeg|png|webp)$/i;
    if (!allowed.test(file.type)) throw new Error(`Unsupported ${kind} file type.`);
    ...
    const response = await fetch("/api/artist-upload", {
      ...
      headers: {
        ...
        "X-File-Kind": kind,
        "X-Artist-Name": encodeURIComponent(artistName),
      },
      body: file,
    });
  ...
  const submitArtistSubmission = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setArtistSubmissionStatus("uploading");
    setArtistSubmissionMessage("");
    try {
      const form = event.currentTarget;
      const data = new FormData(form);
      const artistName = String(data.get("artistName") || "").trim();
      const email = String(data.get("email") || "").trim();
      const songTitle = String(data.get("songTitle") || "").trim();
      const genre = String(data.get("genre") || "").trim();
      const consideration = String(data.get("consideration") || "").trim();
      const streamingLink = String(data.get("streamingLink") || "").trim();
      const audio = data.get("audio");
      const artwork = data.get("artwork");
      const audioFile = audio instanceof File && audio.size ? audio : null;
      const artworkFile = artwork instanceof File && artwork.size ? artwork : null;

      if (!artistName || !email || !songTitle || !genre || !consideration) throw new Error("Please complete all required fields.");
      if (!audioFile && !streamingLink) throw new Error("Please upload your song or provide a streaming/download link.");

      let audioPath = "";
      let artworkPath = "";
      if (audioFile) audioPath = await uploadArtistFile(audioFile, artistName, "audio");
      if (artworkFile) artworkPath = await uploadArtistFile(artworkFile, artistName, "artwork");

      setArtistSubmissionStatus("sending");
      const response = await fetch("/api/artist-submission", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          website: String(data.get("website") || ""),
          artistName, email, country: String(data.get("country") || ""), city: String(data.get("city") || ""),
          genre, songTitle, releaseDate: String(data.get("releaseDate") || ""), social: String(data.get("social") || ""),
          bio: String(data.get("bio") || ""), description: String(data.get("description") || ""), streamingLink, consideration, audioPath, artworkPath,
        }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok) throw new Error(result.error || "The submission could not be completed.");
      setArtistSubmissionStatus("success");
      setArtistSubmissionMessage(result.message || "Submission received. Thank you.");
      form.reset();
    } catch (error) {
      console.error("FOR THE CULTURE artist submission failed:", error);
      setArtistSubmissionStatus("error");
      setArtistSubmissionMessage(error instanceof Error ? error.message : "The submission could not be completed. Please try again.");
    }
  };
  ...
  const ensurePaystackLoaded = () => {
    const existing = document.querySelector('script[src="https://js.paystack.co/v2/inline.js"]') as HTMLScriptElement | null;
    if (existing) return;
      ...
      <section className="culture-platform" id="culture">
        ...
        <div className="culture-platform-shell">
          ...
          <div className="culture-platform-main">
            ...
            <section className="culture-artist-submit" id="culture-submit">
              <div className="culture-submit-intro">
                <span>FOR THE CULTURE MUSIC DISCOVERY</span>
                <h3>YOUR MUSIC.<br /><em>YOUR VOICE.</em></h3>
                <p>Submit your music for consideration by the FOR THE CULTURE team. Selected records may be considered for radio rotation, editorial coverage, or both.</p>
                <small>Every submission is reviewed. Submission does not guarantee airplay, publication or placement.</small>
              </div>
              {artistSubmissionStatus === "success" ? (
                <div className="culture-submit-success">
                  <span>● SUBMISSION RECEIVED</span>
                  <h4>THANK YOU.</h4>
                  <p>{artistSubmissionMessage}</p>
                  <button type="button" className="culture-submit-button" onClick={() => { setArtistSubmissionStatus("idle"); setArtistSubmissionMessage(""); }}>SUBMIT ANOTHER RECORD →</button>
                </div>
              ) : (
                <form className="culture-submit-form" onSubmit={submitArtistSubmission}>
                  <input type="text" name="website" autoComplete="off" tabIndex={-1} aria-hidden="true" className="culture-honeypot" />
                  <div className="culture-submit-choice">
                    <span>CONSIDERATION FOR *</span>
                    <label><input type="radio" name="consideration" value="RADIO" defaultChecked /> RADIO</label>
                    <label><input type="radio" name="consideration" value="EDITORIAL" /> BLOG / EDITORIAL</label>
                    <label><input type="radio" name="consideration" value="BOTH" /> BOTH</label>
                  </div>
                  <div className="culture-submit-fields">
                    <label>ARTIST / STAGE NAME *<input name="artistName" required placeholder="Your artist name" /></label>
                    <label>EMAIL *<input name="email" type="email" required placeholder="you@example.com" /></label>
                    <label>SONG TITLE *<input name="songTitle" required placeholder="Song title" /></label>
                    <label>GENRE *<input name="genre" required placeholder="Afrobeats, Hip-Hop, R&B..." /></label>
                    <label>COUNTRY<input name="country" placeholder="Nigeria" /></label>
                    <label>CITY<input name="city" placeholder="Lagos" /></label>
                    <label>RELEASE DATE<input name="releaseDate" type="date" /></label>
                    <label>SOCIAL LINKS<input name="social" placeholder="Instagram / X / TikTok / website" /></label>
                    <label className="culture-submit-wide">SONG FILE · MP3 / WAV / M4A / AAC · MAX 4 MB<input name="audio" type="file" accept="audio/mpeg,audio/wav,audio/x-wav,audio/mp4,audio/x-m4a,audio/aac,audio/ogg,audio/webm,.mp3,.wav,.m4a,.aac,.ogg" /></label>
                    <label className="culture-submit-wide">OR STREAMING / DOWNLOAD LINK<input name="streamingLink" type="url" placeholder="https://..." /></label>
                    <label className="culture-submit-wide">ARTWORK · JPG / PNG / WEBP · MAX 2 MB<input name="artwork" type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" /></label>
                    <label className="culture-submit-wide">ARTIST BIO<textarea name="bio" rows={3} placeholder="Tell the team who you are and what you represent." /></label>
                    <label className="culture-submit-wide">ABOUT THIS SONG<textarea name="description" rows={3} placeholder="What should the FOR THE CULTURE team know about this record?" /></label>
                  </div>
                  {artistSubmissionStatus === "error" && <div className="culture-submit-error">{artistSubmissionMessage}</div>}
                  <button type="submit" className="culture-submit-button" disabled={artistSubmissionStatus === "uploading" || artistSubmissionStatus === "sending"}>
                    {artistSubmissionStatus === "uploading" ? "UPLOADING RECORD..." : artistSubmissionStatus === "sending" ? "SENDING TO THE CULTURE DESK..." : "SUBMIT MUSIC →"}
                  </button>
                  <small className="culture-submit-destination">SUBMISSIONS ARE REVIEWED BY THE FOR THE CULTURE TEAM · fortheculture184@gmail.com</small>
                </form>
              )}
            </section>

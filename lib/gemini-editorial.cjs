...
async function draftStoriesWithGemini({ apiKey, model = DEFAULT_MODEL, items, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  try {
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        contents: [{ role: 'user', parts: [{ text: prompt }] }],
        generationConfig: { temperature: 0.4, maxOutputTokens: Math.min(7000, Math.max(3000, items.length * 1300)), responseMimeType: 'application/json', responseSchema: batchSchema },
      }),
      signal: controller.signal,
    })
    const responseText = await response.text()
